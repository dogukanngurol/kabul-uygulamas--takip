...
def get_db():
    conn = sqlite3.connect('operasyon_v42.db', check_same_thread=False)
    return conn
...
def filter_dataframe(df, col_prefix=""):
    c1, c2, c3 = st.columns(3)
    workers = ["Hepsi"] + df['assigned_to'].unique().tolist() if 'assigned_to' in df.columns else ["Hepsi"]
    cities = ["Hepsi"] + df['city'].unique().tolist() if 'city' in df.columns else ["Hepsi"]
    
    f_worker = c1.selectbox("Personel Filtresi", workers, key=f"{col_prefix}_fw")
    f_city = c2.selectbox("Şehir Filtresi", cities, key=f"{col_prefix}_fc")
    
    if f_worker != "Hepsi": df = df[df['assigned_to'] == f_worker]
    if f_city != "Hepsi": df = df[df['city'] == f_city]
    return df

SEHIRLER = ["İstanbul", "Adana", "Ankara", "İzmir", "Antalya", "Bursa", "Diyarbakır", "Gaziantep", "Konya", "Mersin", "Samsun"]

# --- 3. GİRİŞ VE OTURUM ---
if 'logged_in' not in st.session_state: st.session_state['logged_in'] = False

if not st.session_state['logged_in']:
    st.title("🛡️ Saha Operasyon v42")
    with st.form("login"):
    ...
    if cp == "🏠 Ana Sayfa":
        ...
        if st.session_state['u_role'] in ['Admin', 'Müdür']:
            ...
            # Haftalık sayaç (Pazartesi başlangıçlı)
            start_of_week = (datetime.now() - timedelta(days=datetime.now().weekday())).strftime("%d/%m/%Y")
            c3.metric("📊 Haftalık İş", conn.execute("SELECT COUNT(*) FROM tasks WHERE updated_at >= ?", (start_of_week,)).fetchone()[0])
        else:
        ...
        s_type = col_f1.selectbox("Durum Bazlı Filtre", ["Hepsi", "Tamamlanan İşler", "Tamamlanamayan İşler"])
        if s_type == "Tamamlanan İşler": df = df[df['result_type'] == 'İŞ TAMAMLANDI']
        elif s_type == "Tamamlanamayan İşler": df = df[df['result_type'].isin(['GİRİŞ YAPILAMADI', 'TEPKİLİ', 'MAL SAHİBİ GELMİYOR'])]
        
        df = filter_dataframe(df, "archive")
        st.dataframe(df, use_container_width=True)
        st.download_button("📊 Excel İndir", to_excel(df), "Arsiv.xlsx")
            ...
            with st.expander(f"🔍 Detay: {r['title']} ({r['assigned_to']})"):
                ...
                if c3.button("❌ Ret", key=f"no_{r['id']}"):
                    if ret_tx:
                        ...
                        conn.commit(); st.rerun()
                    else: st.warning("Ret sebebi girin.")

    elif cp == "📡 TT Onay Bekleyenler":
        st.header("📡 TT Onay Listesi")
        tt_df = filter_dataframe(pd.read_sql("SELECT * FROM tasks WHERE status='Türk Telekom Onayında'", conn), "tt")
        st.dataframe(tt_df)
        st.download_button("📊 Excel İndir", to_excel(tt_df), "TT_Onay.xlsx")
        ...
        h_df = filter_dataframe(h_df, "hakedis")
        st.dataframe(h_df)
        ...
        go_df = filter_dataframe(pd.read_sql("SELECT * FROM tasks WHERE status='Giriş Mail Onayı Bekler'", conn), "go")
        st.dataframe(go_df)
        st.download_button("📊 Excel İndir", to_excel(go_df), "Giris_Onay.xlsx")
        ...
        inv_df = filter_dataframe(pd.read_sql("SELECT * FROM inventory", conn), "inv")
        st.table(inv_df)
